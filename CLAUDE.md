...
[`AGENTS.md`](https://ehrlinger.github.io/hvtiRbootstrap/AGENTS.md),
...
(`/codemap hvtiRbootstrap`). If the codemap looks stale, say so and
...
it, then orient from the repo itself. Start with the fitter contract
comment at the top of `R/fitters.R` — it explains more of this package’s
design than any other file.

## Reading the SAS side

Several rules here only make sense against the macro they port. The
macros live in `~/Documents/macro.library` (`bootstrap.models.sas`,
`bootstrap.summary.sas`, `bootstrap.clusters.sas`). If a question is
“does this match SAS”, read the macro rather than reasoning from the R —
and remember the parity scope:
[`boot_summary()`](https://ehrlinger.github.io/hvtiRbootstrap/reference/boot_summary.md)
is exact, resampling and fitting are not.
